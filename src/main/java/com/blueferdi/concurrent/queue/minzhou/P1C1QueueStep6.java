 ...
 */
package com.blueferdi.concurrent.queue.minzhou;

import static com.blueferdi.concurrent.queue.minzhou.UnsafeDirectByteBuffer.CACHE_LINE_SIZE;
import static com.blueferdi.concurrent.queue.minzhou.UnsafeDirectByteBuffer.alignedSlice;
import static com.blueferdi.concurrent.queue.minzhou.UnsafeDirectByteBuffer.allocateAlignedByteBuffer;
...
public final class P1C1QueueStep6 implements Queue<Integer> {
   ...
   * @param viewMask
   */
  public P1C1QueueStep6(final ByteBuffer buff, final int capacity, byte viewMask) {
    this.capacity = findNextPositivePowerOfTwo(capacity);
    buffy =
        alignedSlice(4 * CACHE_LINE_SIZE + this.capacity << INT_ELEMENT_SCALE,
  ...
  public Integer poll() {
    while (true) {
      ...
      final long offset =
          arrayBase + ((currentHead & mask) << INT_ELEMENT_SCALE);
